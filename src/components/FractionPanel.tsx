            ...
            {[
              ...
              ['Сероводород', 'H₂S', data.h2s],
                ...
                <tr key={i} className="border-b border-slate-800">
                  <td className="py-1.5 text-slate-200">{name as string}</td>
                  <td className="text-right text-slate-400">{formula as string}</td>
                  <td className="text-right font-medium text-slate-200">{(val as number).toFixed(1)}</td>
                </tr>
            ))}
            </tbody>
          </table>
